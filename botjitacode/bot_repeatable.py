import discord
import asyncio
from bot_json import get_member_count_channel, create_member_count_channel, get_trade_notifications_channel, create_trade_notifications_channel
...
async def verifica_canal_membros_on_ready(guild):
    ...
    if not channel_id:
        print(
            f"AVISO: O canal de membros não existe no servidor {guild.name}.")
        return None
